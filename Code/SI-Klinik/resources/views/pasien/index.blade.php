                                    ...
                                    <td>
                                        <div class="d-flex">
                                            ...
                                            <a href="#" class="btn btn-info btn-sm btn-edit"
                                                data-id="{{ $item->id }}"
                                                data-url="{{ url('/pasien') }}/{{ $item->id }}/edit">
                                            ...
                                            <button type="button" class="btn btn-success btn-sm ml-1 btn-reservasi"
                                                data-id="{{ $item->id }}"
                                                data-url="{{ url('/pasien/' . $item->id . '/reservasi/create') }}">
                                                <i class="fas fa-calendar-plus"></i> Reservasi
                                            </button>
                                            <button type="button" class="btn btn-danger btn-sm ml-1 btn-delete"
                                                data-id="{{ $item->id }}"
                                                data-url="/pasien/{{ $item->id }}">
...
<script>
    ...
    $(document).on('submit', '#form-tambah', function(e) {
        ...
        $.ajax({
            ...
            error: function(xhr) {
                alert('Terjadi kesalahan: ' + xhr.responseJSON.message); // Tampilkan pesan error
            },
        });
    });
    ...
    // Script untuk reservasi
    $(document).on('click', '.btn-reservasi', function() {
        var url = $(this).data('url');
        ...
        $.get(url, function(data) {
            ...
            Swal.fire('Error', 'Gagal memuat form reservasi', 'error');
        });
    });

    // Handle submit form reservasi
    $(document).on('submit', '#form-reservasi', function(e) {
        e.preventDefault();
        var form = $(this);
